...
@router.get("/get-user-subs", response_model=StandardResponse[UserSubsResponse])
@autowrap
async def get_user_subs(user_data = Depends(auth.get_user_data_dependency()), db: AsyncSession = Depends(get_db)):
    service = UsersService(db)
    user = await service.get_user_subs(int(user_data["sub"]))
    return user